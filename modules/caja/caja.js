    ...
    async cargarCajaActual() {
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                // Mostrar notificación de sesión activa multi-dispositivo
...
El sistema continúa funcionando normalmente usando cache local.
    ...
    async procesarApertura() {
        ...
        try {
            ...
            if (resultado.existe) {
                // Ya existía una caja abierta (otro dispositivo la creó primero)
                this.cajaActual = resultado.caja;
                await this.cargarVentasTurno();
                await this.cargarMovimientosTurno();
                this.cerrarModalApertura();
                this.renderEstadoCaja();
                this.showNotification('⚠️ Ya tienes una caja abierta desde otro dispositivo', 'warning');
    ...
    calcularDiferencia() {
        ...
        if (esAdmin) {
            if (Math.abs(diferencia) < 0.01) {
                ...
                alert.innerHTML = `✅ Efectivo correcto: ${formatter ? formatter.format(efectivoReal) : '$' + efectivoReal.toFixed(2)}`;
            } else if (diferencia > 0) {
                alert.className = 'diferencia-alert warning';
                alert.innerHTML = `⚠️ Sobrante: ${formatter ? formatter.format(diferencia) : '$' + diferencia.toFixed(2)}`;
            } else {
                alert.className = 'diferencia-alert danger';
                alert.innerHTML = `❌ Faltante: ${formatter ? formatter.format(Math.abs(diferencia)) : '$' + Math.abs(diferencia).toFixed(2)}`;
            }
        } else {
            // Vendedor: Solo confirmar que se ingresó un monto
            alert.className = 'diferencia-alert info';
            alert.innerHTML = `📝 Efectivo contado registrado. El administrador revisará el arqueo.`;
        }
    }
